...
function startAudio() {
	navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
		...
		// 在这里使用录音器
		ms.success(t('chat.startAudio'))
	}).catch((error) => {
		ms.error(t('chat.errorAudio'))
	});
}
...
function endAudio() {
	...
	recorder.value.ondataavailable = (event) => {
		...
		fetchTranscription(formData).then(data => {
			console.log('data', data)
			if (data.status === 'Fail') {
				ms.error(t('chat.errorAudio'))
			} else {
				prompt.value = data.data.toString()
			}
		})
          ...
          <NAutoComplete v-model:value="prompt" :options="searchOptions" :render-label="renderOption">
            <template #default="{ handleInput, handleBlur, handleFocus }">
              ...
              />
					...
					<NButton type="primary" v-if="!recordingFlag" @click="startAudio">
						<template #icon>
              <span class="dark:text-black">
                <SvgIcon icon="mdi:microphone" />
					...
					<NButton type="primary" v-if="recordingFlag" @click="endAudio">
						<template #icon>
              <span class="dark:text-black">
                <SvgIcon icon="ph:microphone-slash-fill" />
              </span>
						</template>
					</NButton>
